...
test("updatePrivateTaskTags rejects non-owner on private task", async () => {
  const originalOwner = state.task.ownerId;
  state.task.ownerId = "user-2";
  const actions = await loadModuleFromRoot<any>("server/actions/tasks");

  await assert.rejects(
    () => actions.updatePrivateTaskTags({ taskId: "task-1", addTagIds: ["tag-1"] }),
    /Forbidden/
  );

  state.task.ownerId = originalOwner;
});
