...
test("rollover creates new tasks for open items and is idempotent", async () => {
  ...
  mock.method(prisma.task, "findMany", async ({ where, select }: any) => {
  ...
  mock.method(prisma.task, "createMany", async ({ data }: any) => {
  ...
  const { rolloverOpenTasks } = await import("@/domain/tasks");
