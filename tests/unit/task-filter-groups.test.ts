...
test("listTaskFilterGroups applies active membership filter for members", async () => {
  ...
  const prismaModule = await loadModuleFromRoot<any>("server/db/prisma");
  const originalFindMany = prismaModule.prisma.group.findMany;
  prismaModule.prisma.group.findMany = async (args: unknown) => {
  ...
  try {
    ...
    await groupsDb.listTaskFilterGroups({
      parishId: "parish-1",
      userId: "user-1",
      role: "MEMBER"
    });
    ...
    prismaModule.prisma.group.findMany = originalFindMany;
  }
});
