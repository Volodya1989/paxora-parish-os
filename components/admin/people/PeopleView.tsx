...
function normalizeMemberRecords(members: ParishMemberRecord[]): ParishMemberRecord[] {
...
export default function PeopleView({
  ...
  const [displayedMembers, setDisplayedMembers] = useState(() => normalizeMemberRecords(members));
  const [isSearching, setIsSearching] = useState(false);
  const activeSearchRequest = useRef(0);

  useEffect(() => {
    setDisplayedMembers(normalizeMemberRecords(members));
  }, [members]);
    ...
    void (async () => {
      try {
        ...
        if (activeSearchRequest.current === requestId) {
          setDisplayedMembers(normalizeMemberRecords(payload.members ?? []));
        }
      } catch {
