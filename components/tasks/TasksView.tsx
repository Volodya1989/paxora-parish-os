    ...
    <div className="section-gap">
      ...
      {showPrivateTagChips ? (
        <div className="flex flex-wrap items-center gap-2">
          ...
          {privateTagFilter !== "all" ? (
            <button
              type="button"
              onClick={() => setPrivateTagFilter("all")}
              className="rounded-full border border-mist-200 bg-white px-3 py-1 text-xs font-semibold text-ink-500 hover:bg-mist-50"
            >
              {t("tasks.tags.filters.clear")}
            </button>
          ) : null}
        </div>
      ) : null}
