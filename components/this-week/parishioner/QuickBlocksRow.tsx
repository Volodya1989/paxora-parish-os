      ...
      {blocks.map((block) => (
        ...
        >
          ...
          <div className="min-w-0 space-y-0.5">
            ...
            {block.statusDots ? (
              <p className="flex flex-wrap items-center gap-1 text-xs font-medium text-ink-700" aria-label={block.summary}>
                <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-green-500" />{block.statusDots.done}</span>
                <span className="text-ink-400">·</span>
                <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-yellow-400" />{block.statusDots.inProgress}</span>
                <span className="text-ink-400">·</span>
                <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-blue-500" />{block.statusDots.open}</span>
              </p>
            ) : (
