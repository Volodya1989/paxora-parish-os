...
3. IOS-A3 iOS pipeline baseline:
   ...
   - On non-macOS runners, this uses Capacitor copy/skip behavior (not full `sync`) to avoid pod install/Xcode dependency failures
...
This is intentional for IOS-D4 scope: deterministic wrapper asset validation + evidence bundle generation in broadly available CI environments.
...
- `run-metadata.txt`
  ...
  - `ios_pipeline_mode` marker (`copy_or_skip_non_macos` on ubuntu lane)
