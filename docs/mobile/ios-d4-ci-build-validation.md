...
3. IOS-A3 iOS pipeline baseline:
   ...
   - On non-macOS runners, this uses Capacitor `copy` mode (not `sync`) to avoid pod install/Xcode dependency failures
...
- `run-metadata.txt`
  ...
  - `ios_pipeline_mode` marker (`copy_non_macos` on ubuntu lane)
