...
| IOS-B5 | **Partial** | `docs/mobile/app-store-connect-metadata.md` defines canonical URLs (support, privacy, terms, deletion reference). `lib/mobile/appStoreMetadata.ts` codifies `support@paxora.com` + URL builder. `components/profile/DeleteAccountCard.tsx` references `APP_STORE_SUPPORT_EMAIL` and `APP_STORE_PATHS`. **However:** URLs use `<public-site-domain>` placeholder — production domain not finalized. Final legal sign-off not recorded. | **P0** | Blocks ASC legal field entry with confidence. Review rejection if URLs 404 or mismatch. |
